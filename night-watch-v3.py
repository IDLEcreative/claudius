...
def can_spawn_fixer() -> tuple[bool, str]:
    ...
    # CONSERVATIVE LIMITS to prevent fork bombs:
    # - Max 2 total claude processes (1 parent Claudius + 1 worker)
    # - Need at least 4GB free RAM
    # - This ensures only ONE auto-fix runs at a time

    if available_gb < 4:
        return False, f"Low memory: {available_gb:.1f}GB available (need 4GB+)"

    if current_claude_count >= 2:
        return False, f"Worker limit: {current_claude_count} claude sessions (max 2)"
...
# Flag file to coordinate with Watchdog
NIGHT_WATCH_FLAG = Path("/tmp/night-watch-active")

LOG_DIR.mkdir(exist_ok=True)
REPORTS_DIR.mkdir(exist_ok=True)


def set_night_watch_active(active: bool):
    """Set flag to tell Watchdog we're running auto-fix."""
    if active:
        NIGHT_WATCH_FLAG.write_text(f"active since {datetime.now().isoformat()}\n")
        print(f"[NIGHT-WATCH] Set active flag: {NIGHT_WATCH_FLAG}")
    else:
        NIGHT_WATCH_FLAG.unlink(missing_ok=True)
        print(f"[NIGHT-WATCH] Cleared active flag")
...
def spawn_claude_fixer(issue_type: str, issue_details: str, timeout: int = 600) -> dict:
    """Spawn a Claude agent to fix an issue using the async SDK.

    Uses claude-agent-sdk for proper async execution - won't block.
...
2. EDIT/WRITE to make the fix
3. RUN typecheck to verify: `npx tsc --noEmit --skipLibCheck 2>&1 | head -20`
4. If errors, fix them and verify again
5. GIT COMMIT your changes: `git add -A && git commit -m "fix: auto-fix {issue_type}"`

**Rules:**
- Working directory is /opt/omniops
- Make minimal, targeted changes
- Run the typecheck after EVERY change to verify
- ALWAYS commit your successful fixes
...
Start now."""
    ...
    async def run_fix():
        ...
        try:
            ...
            result["output"] = "\n".join(output_parts)
            result["success"] = bool(output_parts)
            print(f"[SPAWN] Claude completed. Output length: {len(result['output'])}")
        except asyncio.TimeoutError:
            result["output"] = f"Timed out after {timeout}s"
            print(f"[SPAWN] Claude timed out")
        except Exception as e:
            result["output"] = f"Error: {e}"
            print(f"[SPAWN] Error: {e}")

    print(f"[SPAWN] Starting Claude agent for {issue_type}...")
    try:
        asyncio.run(asyncio.wait_for(run_fix(), timeout=timeout))
    except asyncio.TimeoutError:
        result["output"] = f"Timed out after {timeout}s"
        print(f"[SPAWN] Timed out after {timeout}s")
    except Exception as e:
        result["output"] = f"Error: {e}"
        print(f"[SPAWN] Error: {e}")
...
def run_auto_fix(findings: List[dict]) -> List[dict]:
    """Run auto-fix on eligible findings with proper coordination and backpressure.
    ...
    """
    fix_results = []
    max_concurrent_failures = 3  # Circuit breaker threshold
    consecutive_failures = 0

    # Set flag to tell Watchdog we're in auto-fix mode
    set_night_watch_active(True)
    ...
    try:
        ...
        loc_findings = [f for f in findings if "LOC" in f["title"] or "exceed" in f["title"].lower()]
        for finding in loc_findings[:1]:  # Max 1 LOC finding per run (contains multiple files)
            details = finding.get("details", "")
            lines = details.strip().split('\n')
            for line in lines[:3]:  # Max 3 files per run to avoid overload
                # Check circuit breaker
                if consecutive_failures >= max_concurrent_failures:
                    print(f"[AUTO-FIX] Circuit breaker tripped after {consecutive_failures} failures, stopping")
                    fix_results.append({
                        "type": "circuit_breaker",
                        "success": False,
                        "details": f"Stopped after {consecutive_failures} consecutive failures"
                    })
                    return fix_results
                ...
                if not can_spawn:
                    print(f"[AUTO-FIX] Backpressure: {reason}")
                    # Wait and retry once
                    time.sleep(60)
                    can_spawn, reason = can_spawn_fixer()
                    if not can_spawn:
                        print(f"[AUTO-FIX] Still blocked: {reason}, skipping fix")
                        fix_results.append({
                            "type": "backpressure_skip",
                            "success": False,
                            "details": reason
                        })
                        continue
                ...
                if len(parts) >= 2:
                    ...
                    if file_path.endswith('.ts') or file_path.endswith('.tsx'):
                        print(f"[AUTO-FIX] Fixing LOC violation in {file_path} ({loc_count} lines)...")
                        result = auto_fix_with_clode(
                            "LOC_VIOLATION",
                            f"File: {file_path}\nCurrent lines: {loc_count}\nLimit: 300\nAction: Split into smaller, focused modules. Extract helper functions to new files."
                        )
                        fix_results.append({
                            "type": "loc_split",
                            "file": file_path,
                        ...
                        # Throttle between spawns - 60s to let system recover
                        print(f"[AUTO-FIX] Waiting 60s before next fix (backpressure)...")
                        time.sleep(60)
        ...
        for finding in ts_findings[:2]:  # Max 2 TS fix sessions
            ...
            if consecutive_failures >= max_concurrent_failures:
                print(f"[AUTO-FIX] Circuit breaker tripped, stopping TS fixes")
                break

            # Backpressure check
            can_spawn, reason = can_spawn_fixer()
            if not can_spawn:
                print(f"[AUTO-FIX] Backpressure for TS fix: {reason}, waiting 60s...")
                time.sleep(60)
                can_spawn, reason = can_spawn_fixer()
                if not can_spawn:
                    print(f"[AUTO-FIX] Still blocked, skipping TS fix")
                    continue
