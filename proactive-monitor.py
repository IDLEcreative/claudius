...
DEPLOY_LOCKFILE = "/opt/omniops/.deployment.lock"
DEPLOY_GRACE_PERIOD = 180  # Seconds to suppress alerts during/after deployment
...
class ProactiveMonitor:
    ...
    def is_deployment_in_progress(self) -> bool:
        """Check if a deployment is currently in progress or recently finished"""
        if os.path.exists(DEPLOY_LOCKFILE):
            try:
                mtime = os.path.getmtime(DEPLOY_LOCKFILE)
                age = time.time() - mtime
                # If lockfile exists and is recent, deployment in progress
                if age < DEPLOY_GRACE_PERIOD:
                    logger.info(f"Deployment in progress (lockfile age: {int(age)}s)")
                    return True
            except:
                pass
        return False
    ...
    def check_container_health(self):
        ...
        # Skip alerts during deployments (live/next swap)
        if self.is_deployment_in_progress():
