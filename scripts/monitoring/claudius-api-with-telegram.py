...
MAX_CONCURRENT_CLAUDE_SESSIONS = 5  # With 8GB RAM, can handle more parallel work
...
def call_brain_api(operation: str, params: dict = None) -> dict:
    """Call Brain MCP via HTTP API (inside Docker container)."""
    if not ADMIN_SECRET:
    ...
    try:
        ...
        logger.error(f"Brain API HTTP error: {e.code}")
        return {"success": False, "error": f"HTTP {e.code}"}
    except urllib.error.URLError as e:
        logger.error(f"Brain API connection error: {e.reason}")
...
def invoke_claudius(prompt: str, conversation_history: list = None, session_id: str = None) -> dict:
    ...
    """
    global active_claude_sessions

    # Check if we can acquire a slot (non-blocking check first)
    with session_lock:
        if active_claude_sessions >= MAX_CONCURRENT_CLAUDE_SESSIONS:
            logger.warning(f"Concurrency limit reached ({active_claude_sessions}/{MAX_CONCURRENT_CLAUDE_SESSIONS})")
            return {
                "response": "I'm currently handling multiple requests. Please wait a moment and try again.",
                "session_id": session_id
            }

    # Acquire semaphore (will block if at limit, but we checked above)
    # 30 min wait for a slot - long tasks shouldn't block new work forever
    acquired = claude_semaphore.acquire(timeout=1800)
    if not acquired:
        return {
    ...
    with session_lock:
        ...
        # Track what this session is working on
        active_session_details[tracking_id] = {
            "started_at": datetime.now().isoformat(),
            "prompt_preview": prompt[:100] + "..." if len(prompt) > 100 else prompt,
            "session_id": session_id
        }
        logger.info(f"Claude session started ({active_claude_sessions}/{MAX_CONCURRENT_CLAUDE_SESSIONS} active) - {tracking_id}")

    try:
        # Build command with JSON output
        cmd = ["claude", "--print", "--output-format", "json", "--permission-mode", "bypassPermissions"]
        ...
        # Format conversation history as fallback context (for new sessions or context)
        history_context = ""
        if conversation_history and not session_id:
            # Only include history context for new sessions (--resume handles its own context)
            history_context = format_history_for_context(conversation_history)
            logger.info(f"Including {len(conversation_history)} messages of conversation history")
...
IMPORTANT: If the user refers to something from the conversation history above, use that context to provide a relevant response. The history shows recent messages from this conversation."""

        # Add IS_SANDBOX=1 to allow --permission-mode bypassPermissions as root
        env = os.environ.copy()
        env["IS_SANDBOX"] = "1"
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=86400,  # 24 hour timeout - let him work as long as needed
            cwd=CLAUDIUS_DIR,
            env=env
        )
        ...
        if stdout:
            try:
                ...
                new_session_id = json_response.get("session_id")
                logger.info(f"Claude session ID: {new_session_id}")
                return {
                ...
                logger.warning("Could not parse JSON response, using raw output")
                return {
                    "response": stdout,
                    "session_id": session_id  # Keep existing session ID if any
                }
        ...
        with session_lock:
            active_claude_sessions -= 1
            active_session_details.pop(tracking_id, None)
            logger.info(f"Claude session ended ({active_claude_sessions}/{MAX_CONCURRENT_CLAUDE_SESSIONS} active) - {tracking_id}")
...
class ClaudiusHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            ...
            with session_lock:
                sessions = list(active_session_details.values())
                self.send_json({
                    "active_sessions": active_claude_sessions,
                    "max_sessions": MAX_CONCURRENT_CLAUDE_SESSIONS,
                    "sessions": sessions,
                    "status": "busy" if active_claude_sessions > 0 else "idle"
                })
